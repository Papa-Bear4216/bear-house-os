...
async function notifyUid(opts: {
  ...
  const { uid, title, body, data, url } = opts;

  const db = getAdminFirestore();
  const tokenSnap = await db.collection('users').doc(uid).collection('fcmTokens').get();

  if (tokenSnap.empty) {
    return { sent: 0, failed: 0, reason: 'no tokens registered for this user' };
  }
  ...
  if (!tokens.length) return { sent: 0, failed: 0, reason: 'no valid tokens' };
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const result = await notifyUid({ uid, title, body, data, url });
...
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    const results = await Promise.allSettled(
      (uids as string[]).map(uid => notifyUid({ uid, title, body, data, url }))
    );
